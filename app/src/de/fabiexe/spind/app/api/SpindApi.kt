package de.fabiexe.spind.app.api

import de.fabiexe.spind.*
import de.fabiexe.spind.app.data.DecodedVault
import de.fabiexe.spind.app.data.PasswordGroup
import de.fabiexe.spind.app.data.SecurityQuestion
...
class SpindApi(ktorEngine: HttpClientEngineFactory<*>, private val storage: VaultStorage) {
    ...
    suspend fun getVault(vault: VaultInfo, secret: String): Either<ByteArray, ErrorResponse> {
        try {
            val localRevision = storage.loadRevision(vault.url)
            ...
            return when (response.status) {
                HttpStatusCode.OK -> {
                    ...
                    storage.save(vault, serverRevision, data)
                    Either.Left(data)
                }
                HttpStatusCode.NotModified -> {
                    // Server has same or older content
                    val data = storage.loadData(vault.url)
                    if (data != null) {
                        Either.Left(data)
                    } else {
    ...
    suspend fun getVaultRecovering(vault: VaultInfo, backupSecret: String): Either<ByteArray, ErrorResponse> {
        try {
    ...
    suspend fun unlockVaultUsingPassword(vault: VaultInfo, password: String): Either<UnlockedVault, ErrorResponse> {
    ...
    suspend fun uploadVault(unlockedVault: UnlockedVault): ErrorResponse? {
        ...
        try {
            val revision = (storage.loadRevision(vault.url) ?: 0) + 1
            ...
            return if (response.status.isSuccess()) {
                storage.save(vault, revision, data)
                null
            } else {
                response.body()
            }
    ...
    suspend fun getSecurityQuestions(vault: VaultInfo): Either<List<String>, ErrorResponse> {
        try {
    ...
    suspend fun unlockVaultUsingSecurityQuestions(vault: VaultInfo, answers: List<String>): Either<UnlockedVault, ErrorResponse> {
